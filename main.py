...
def parse_arguments():
    ...
    parser.add_argument('--eval', help='.tsv file with sentences for bias evaluation', required=True)
    parser.add_argument('--tune', help='file with sentences for fine-tuning', required=False)
...
def fine_tune(model, dataloader, epochs, tokenizer, device):
    ...
    # Note: AdamW is a class from the huggingface transformers library (as opposed to pytorch)
